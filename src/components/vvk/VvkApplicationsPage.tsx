...
export function VvkApplicationsPage() {
  const [applications, setApplications] = useState<VvkApplicationRecord[]>([]);

  useEffect(() => {
    const load = () => setApplications(getVvkApplicationsClient());
    load();
                  ...
                  {applications.map((item) => (
                    <tr key={item.id} className="even-table montserrat border-b text-center">
                      ...
                      <td className="px-6 py-4">
                        <p className="text-green-500">{item.requestStatus}</p>
                      </td>
                      <td className="px-6 py-4">
                        <p className="text-indigo-600">{item.responseStatus}</p>
                      </td>
                      ...
                      <td className="px-6 py-4">
                        <Link href={`/vvk/applications/${item.id}`} className="underline cursor-pointer">
                          Просмотр
                        </Link>
                      </td>
                    </tr>
